...
class orderController {
        ...
        try {
            ...
            for (let i = 0; i < products.length; i++) {
                ...
                authorOrderData.push({
                    ...
                    products: storePro,
                    price: pri,
                    payment_status: order.payment_status,  // from main order
                    shippingInfo: shippingInfo,           // from req.body
                    delivery_status: order.delivery_status, // from main order
                    date: tempDate
                })
            }
    ...
    seller_order_status_update = async (req, res) => {
        ...
        console.log('---seller_order_status_update backend---')
        console.log('Order ID:', orderId)
        console.log('Status to update:', status)

        try {
            const updatedOrder = await authOrderModel.findByIdAndUpdate(
                orderId,
                { delivery_status: status },
            ...
            responseReturn(res, 200, { message: 'Order status changed successfully', updatedOrder })
        } catch (error) {
            console.log('get admin order status error ' + error.message)
            responseReturn(res, 500, { message: 'Internal server error' })
        }
